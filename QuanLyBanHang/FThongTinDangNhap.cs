...
namespace QuanLyBanHang
{
    public partial class FThongTinDangNhap : Form
    {
        ...
        public FThongTinDangNhap(DangNhap tk)
        {
            InitializeComponent();
            Dn = tk;
            loadThongTin();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DangNhap d = DangNhapDAO.Instance.getByTaiKhoan(tbTaiKhoan.Text);
            if(d!=null&&d.MaNV!=Dn.MaNV)
            {
                MessageBox.Show("Tài khoản đã tồn tại !", "Nhắc nhở");
                return;
            }
            DangNhapDAO.Instance.suaByTaiKhoan(tbTaiKhoan.Text, tbMatKhau.Text, Dn.TaiKhoan);
            Dn = DangNhapDAO.Instance.getByTaiKhoan(tbTaiKhoan.Text);
